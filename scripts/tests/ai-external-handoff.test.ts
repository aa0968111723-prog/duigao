/**
 * PR-GAP-07：疊在 #88 之上。不實作 DI schema，不複製 0027–0028 到 main。
 *
...
test("G7-07: 本批沒有把 0027–0028 複製走；全目標未完成", () => {
  const migrations = readdirSync(resolve(ROOT, "supabase/migrations")).filter((name) => name.endsWith(".sql"));
  assert.ok(migrations.includes("0027_design_knowledge.sql"));
  assert.ok(migrations.includes("0028_design_research_usage.sql"));
