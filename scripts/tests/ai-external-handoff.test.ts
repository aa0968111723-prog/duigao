/**
 * PR-GAP-07 restack：疊在 #118（0029–0030）之上。不實作 DI schema，不複製 SQL 到 main。
 *
...
test("G7-06: 金鑰不在前端；不從 client 把房間內容丟給 Perplexity", () => {
  ...
  const config = src("src/cloud/config.ts");
...
test("G7-07: 本批沒有把 DI SQL 複製走；檔名是 0029–0030；全目標未完成", () => {
  const migrations = readdirSync(resolve(ROOT, "supabase/migrations")).filter((name) => name.endsWith(".sql"));
  assert.ok(migrations.includes("0029_design_knowledge.sql"));
  assert.ok(migrations.includes("0030_design_research_usage.sql"));
  assert.equal(migrations.includes("0027_design_knowledge.sql"), false, "舊 #88/#104 檔名不得留在此 restack");
  assert.equal(migrations.includes("0028_design_research_usage.sql"), false, "舊 #88/#104 檔名不得留在此 restack");
  assert.equal(migrations.includes("0022_discussion_author_integrity.sql"), false);

  const handoff = src("docs/cursor-gap-remediation/AI_EXTERNAL_HANDOFF.md");
  assert.match(handoff, /HANDOFF \/ INCOMPLETE/);
  assert.match(handoff, /0029/);
  ...
  assert.equal(existsSync(resolve(ROOT, "docs/cursor-gap-remediation/AI_EXTERNAL_HANDOFF.md")), true);
});

test("G7-08: #97 已在 main 修 Canva parser；本棧不重做、也不把 #88 schema 複製到 main", () => {
  const hasParser = existsSync(resolve(ROOT, "src/cloud/apiResponse.ts"));
  const canva = src("src/cloud/canva.ts");
  if (hasParser) {
    assert.match(canva, /parseFunctionPayload/);
  } else {
    assert.match(canva, /as CanvaBridgeHealth/, "#88 樹上仍是舊 cast；誠實留下，修在已合併的 #97 main");
  }
});
