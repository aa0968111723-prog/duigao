...
test("older duplicate loses; newer update replaces; delete removes the row", () => {
...
test("wiring: room sync rejects bad payloads and discussion no longer whole-room reloads", () => {
  ...
  assert.match(sync, /onDiscussionUpsert/);
  assert.doesNotMatch(
    sync,
  ...
  assert.match(app, /applyDiscussionRealtime/);
  assert.match(app, /onDiscussionPatch/);
});
