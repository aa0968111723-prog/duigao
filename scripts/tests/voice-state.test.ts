 ...
 * Restacked onto the room shell (#112) so nine-state and V-04 Leave coexist.
 */
...
test("negative: permission-denied is distinct from connection-failed", () => {
  ...
  assert.doesNotMatch(voicePhaseMessage("connection-failed"), /已連線/);
  assert.doesNotMatch(voicePhaseMessage("service-not-configured"), /已連線/);
...
test("legacy dock mapping: reconnecting is connecting (not live); V-07 still mutes first", () => {
  ...
  assert.equal(
    ...
    true,
    "legacy live-only dock would hide leave while mic is still open",
  );
  assert.equal(
...
test("V-04: dock Leave stays visible for reconnecting phase on this stack", () => {
  ...
  assert.match(discussion, /voiceDockShowsLeave\(api\.voice\.phase/);
});
...
test("hook and liveVoice use the truthful machine; RoomDiscussion reads phase for Leave", () => {
  const hook = readFileSync(resolve(ROOT, "src/hooks/useVoiceRoom.ts"), "utf8");
  const live = readFileSync(resolve(ROOT, "src/features/voice/liveVoice.ts"), "utf8");
  ...
  assert.match(token, /parseFunctionPayload/);
