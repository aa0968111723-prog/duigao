...
test("nine-state parser reuses the shared #97 SPA gate; client still calls parseFunctionPayload", () => {
  const machine = readFileSync(resolve(ROOT, "src/features/voice/voiceState.ts"), "utf8");
  const token = readFileSync(resolve(ROOT, "src/cloud/voiceToken.ts"), "utf8");
  assert.match(machine, /parseFunctionPayload/);
