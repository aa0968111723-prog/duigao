...
test("edge UPDATE：版本前進才替換（0022 之後 label/handle 可改）", () => {
  ...
  const first = applyBoardPatches(nodes, before, 0, [
  ...
  const stale = applyBoardPatches(nodes, first.edges, 0, [
...
test("edge INSERT：新的線照常進來，且不重複", () => {
  const first = applyBoardPatches(nodes, [], 0, [{ type: "edge-insert", edge: edge() }]);
  assert.equal(first.edges.length, 1);
  const again = applyBoardPatches(nodes, first.edges, 0, [{ type: "edge-insert", edge: edge() }]);
  assert.equal(again.edges.length, 1, "同一條線的重複事件不得長出第二條");
});

test("edge DELETE：刪掉就是刪掉", () => {
  const start = applyBoardPatches(nodes, [edge()], 0, [{ type: "edge-delete", id: "e1" }]);
