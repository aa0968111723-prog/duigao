...
test("H-02: 空白板進場在 #78 樹上已誠實（本批不重寫該 UI）", () => {
...
test("H-05: 本批沒有新增 whiteboard SQL，也沒宣告全目標完成", () => {
  const migrations = readdirSync(resolve(ROOT, "supabase/migrations")).filter((name) => name.endsWith(".sql")).sort();
  assert.ok(migrations.includes("0022_whiteboard_canonical_columns.sql"), "#78 樹上仍是他們的 0022 whiteboard（勿複製到 main）");
  assert.ok(migrations.includes("0026_whiteboard_freehand.sql"));
  assert.equal(
