...
try {
  ...
  await page.getByLabel("房間討論").focus();
  await page.waitForFunction(
    () => document.querySelector('[data-testid="multi-branch-room"]')?.getAttribute("data-composer-active") === "true",
    null,
    { timeout: 5000 },
  );
  const composing = await page.evaluate(() => ({
    ...
    ai: document.querySelectorAll('[data-testid="room-ai-launcher"]').length,
    overview: document.querySelectorAll('[data-testid="open-overview-pane"]').length,
    sheet: document.querySelectorAll('[data-testid="room-more-sheet"]').length,
  }));
  check("390 討論輸入聚焦時隱藏更多／總覽／AI", composing.more === 0 && composing.ai === 0 && composing.overview === 0 && composing.sheet === 0);
  await page.screenshot({ path: join(ARTIFACTS, "gap04-composer-hide-390x844.png"), fullPage: false });
