...
try {
  ...
  check("活動房只有一個 AI 入口，且藏在更多裡", await page.getByTestId("room-ai-launcher").count() === 1);
  await page.getByTestId("room-ai-launcher").click();
  await page.waitForSelector('[data-testid="room-ai-sheet"]', { timeout: 10000 });
  check("手機 AI 入口是 bottom sheet", await page.locator('[data-testid="room-ai-sheet"]').isVisible());
