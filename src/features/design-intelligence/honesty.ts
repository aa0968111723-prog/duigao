/**
 * GAP-07 AI／外部工具誠實契約（疊在 main #114 DI 上，不重寫 schema / migration）。
 *
 * 三條產品句：
 ...
 *
 * SPA 判定重用 #97 `apiResponse.looksLikeSpaHtml`，不另寫一套。
 */
import { looksLikeSpaHtml as sharedLooksLikeSpaHtml } from "../../cloud/apiResponse";
...
/** Re-export the shared #97 gate so DI tests and callers share one detector. */
export function looksLikeSpaHtml(value: unknown, contentType?: string | null): boolean {
  return sharedLooksLikeSpaHtml(value, contentType);
}
