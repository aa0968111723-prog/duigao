...
看板本身鋪 AiOS 標誌自己的紙底（`#F2EBE4` 頁面 / `#FBF6F2` 卡片 / `#E7DED5` 面板 /
`#D9CEC4` 分隔線），墨色三階 `#201B15` / `#554B41` / `#736858` —— 三階在紙底與卡片上
都過 WCAG AA 4.5:1（14.5 / 7.2 / 4.6）。

**四張手機稿維持暗色**，因為那是 app 真正的樣子（`src/styles.css` 是 `color-scheme: dark`）。
把手機稿改亮就等於宣稱產品是亮色的，那不是現況。深色裝置稿擺在亮底看板上，
本來就是設計看板該有的樣子。

元件板同理：外框是紙底，但每個元件仍待在深色的樣本磚上，
這樣按鈕、圖釘、修改點卡才是在它們真正的底色上被檢視。
