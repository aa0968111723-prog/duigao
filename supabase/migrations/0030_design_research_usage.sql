...
--
-- 設計：**對 client 而言是 append-only 之外再退一步 —— 連 insert 都拿不到**。
--
-- 措辭要精確（對抗審查點過這個檔頭一次）：`service_role` 有 `grant all`，
-- 所以它改得動也刪得掉。真正的保證是「**client 一行都寫不進去也改不掉**」，
-- 而不是「這張表在資料庫層面不可變」。edge function 只做 insert，
-- 那是流程上的約定，不是資料庫強制的。
--
...
--     TRUNCATE 也一併收回：RLS 不管 TRUNCATE，留著等於配額可以被清零。
