...
| Share / invite fragment secret leakage | **NOT_A_REMAINING_P0** | `buildInviteUrl` writes `#…` only. `readInviteFromUrl` reads invite from `location.hash` only. Existing `collaboration-workspace.test.ts` already asserts query `?invite=` is ignored. No new leak found. |
